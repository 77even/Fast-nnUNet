...
def run_distillation_training(dataset_id, 
                             ...
                             epochs=1000):
    """
    Run nnUNet knowledge distillation training.
    ...
    # If teacher_model_folder is None, auto-construct
    if teacher_model_folder is None:
        teacher_model_folder = join(nnUNet_results, dataset_name, "nnUNetTrainer__nnUNetPlans__" + configuration)
        print(f"Teacher model folder not specified, using default: {teacher_model_folder}")

    # Ensure necessary files exist
    if not os.path.exists(teacher_model_folder):
        raise FileNotFoundError(f"Teacher model folder does not exist: {teacher_model_folder}")
    ...
    if teacher_folds is None:
        ...
        if len(teacher_folds) == 0:
            print(f"Warning: No available teacher model folds found, will use fold_0")
            teacher_folds = [0]
        else:
            print(f"Detected {len(teacher_folds)} available teacher model folds: {teacher_folds}")
    else:
        ...
        for teacher_fold in teacher_folds:
            ...
            if not os.path.exists(teacher_checkpoint_path):
                raise FileNotFoundError(f"Teacher model checkpoint does not exist: {teacher_checkpoint_path}")
    ...
    if not os.path.exists(plans_file):
        raise FileNotFoundError(f"Plans file does not exist: {plans_file}, please ensure preprocessing step has been run")
